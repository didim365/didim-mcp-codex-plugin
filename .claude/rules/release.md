...
- 커밋 메시지는 기존 관례인 Conventional Commits를 따른다. 실제 사용된 형태:
  `feat: `, `feat(setup): `, `feat(skill): `, `fix(scripts): `.

## 버전

- 사용자에게 보이는 변경(스크립트 동작, Skill 내용, 매니페스트)을 커밋할 때는
  `plugins/didim-mcp/.codex-plugin/plugin.json`의 `version`을 함께 올린다. 지금까지 모든 기능
  커밋이 `0.1.0 → 0.1.4`로 1건씩 올렸다. 버전이 안 오르면 사용자 쪽 업데이트 판정이 어렵다.
